...
In case you have any questions, do not hesitate to contact us using the following e-mail: petronio.candido@ifnmg.edu.br

- Our QNCA approach mixes the power of Quantum Neural Networks(QNN) with the Quantum Cellular Automata (QCA), using Parameterized Quantum Circuits (PQC) and Variational Quantum Algorithms (VQA) to teach recursive patterns
- The objective of this research is to assess the learning capacity of parameterized recurrent quantum circuits in a unidimensional Cellular Automaton with a simple neighborhood function.
- The Quantum Neural Cellular Automata is designed to emulate classical CA, but with learned parameters that will be trained to adjust to
- CA are well known for its capacity to produce complex patterns with a simple set of unidimensional neighborhood rules. The recursive application of these
- Dynamical Processes, Chaos. Patterns generated by CA are intrinsically spatiotemporal, with simultaneous dependence on the spatial neighborhood of each cell and their temporal evolution. 
- Neural Cellular Automata, on the other hand, are recently reaching more attention
- The objective of this research is to investigate and assess the 
...
- While using ancilla quibts increases the width of the circuit, it grants the correct evolution of the cellular automaton.

- The length of the circuit is directly affected by the number of iterations T, which implies the number of repetitions of the unitary.

- Learning the shared circuit parameters is a challenging task due to the iterative and recursive nature of the Cellular Automata. An analogy for this complexity is to think that the same parameters are repeated vertically (among each set of neighbor qubits) and repeated horizontally, for each iteration of the algorithm.
